/**
 * Cabin Booking Page Component
 * Allows users to view Cabin availability by location and book/join waitlist
 */
...
const CabinBooking = () => {
  ...
  const fetchLocations = async () => {
    try {
      const response = await cabinAPI.getLocations();
  ...
  const fetchHospitals = async (location = '', hospital = '') => {
    ...
    try {
      ...
      const response = await cabinAPI.getAll(params);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    setError('');
    try {
      const response = await cabinAPI.book({
      ...
      setError(err.response?.data?.message || 'Failed to book Cabin');
    } finally {
  ...
  const handleWaitlistSubmit = async (e) => {
    ...
    setError('');
    try {
      const response = await cabinAPI.joinWaitlist({
        hospital_id: selectedHospital.hospital_id,
            ...
            <div key={location} className="location-section">
              ...
              <div className="hospitals-grid">
                {hospitals.map((hospital) => (
                  <div key={hospital.cabin_id} className="hospital-card">
